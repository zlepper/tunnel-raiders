use bevy_prototype_debug_lines::{DebugLines, MAX_POINTS};
        ...
        for (tile_coord, tile) in nav_mesh.get_tiles().iter() {
            ...
            for poly in tile.polygons.iter() {
                ...
                for i in 0..indices.len() {
                    ...
                    if (a - cam.translation()).length() > 100. || (b - cam.translation()).length() > 100. {
                        continue;
                    }

                    if lines.positions.len() < MAX_POINTS {
            ...
            for vertex in tile.vertices.iter() {
                ...
                if (*vertex - cam.translation()).length() > 100. {
                    continue;
                }
                if lines.positions.len() < MAX_POINTS {

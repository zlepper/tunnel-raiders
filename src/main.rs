...
use crate::errands::{ErrandsPlugin, Miner, PlayerMovable, WorkerPriorities};
...
pub trait AppAssetExtensions {
    fn load_assets<T: AssetCollection>(&mut self) -> &mut Self;
}

impl AppAssetExtensions for App {
    fn load_assets<T: AssetCollection>(&mut self) -> &mut Self {
        self.add_collection_to_loading_state::<_, T>(GameState::Loading)
    }
}
